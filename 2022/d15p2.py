...
data= """Sensor at x=3428425, y=2345067: closest beacon is at x=3431988, y=2379841
...
data = parse(data, ["Sensor at x=",Int(),", y=",Int(),": closest beacon is at x=",Int(),", y=", Int()])
points = []
all_stuff = set()
...
def get_points_at_distance(p,d):
    #find all the points which are distance d from p
    ps = set()
...
def t(a):
    print(a)
    S, B, d = a
    for p in get_points_at_distance(S, d+1):
        ...
        if not excluded:
            ...
            return (p,p.x*4000000 + p.y)


with Pool(10) as p:
    print(p.map(t, points))
