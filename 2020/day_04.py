...
data="""iyr:2010 ecl:gry hgt:181cm
...
NUMBERS =  [str(j) for j in range(0,10)]
LETTERS = ['a','b','c','d','e','f']

def check_colour(hcl):
    if hcl[0]!="#":
        return False
    if all(i in NUMBERS+LETTERS for i in hcl[1:]):
        return True
...
def check_pid(pid):
    return len(pid) == 9 and all(i in NUMBERS for i in pid)
...
def run(data):
    ...
    for data in all_data:
        if all(a in data.keys() for a in ('byr','iyr','eyr','hgt','hcl','ecl','pid')):
            if 1920<=int(data['byr'])<=2002:
                if 2010<=int(data['iyr'])<=2020:
                    if 2020<=int(data['eyr'])<=2030:
                        if check_hgt(data['hgt']):
                            if check_colour(data['hcl']):
                                if check_eye(data['ecl']):
