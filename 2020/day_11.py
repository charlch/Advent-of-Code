...
L.LLLLLL.L
L.LLLLL.LL"""
data="""LLLLLLLLL.LLLLL.LLLLLLLLL.LLLLLLLLLLLL.LLLLLL.LL.LLL.LL.L.LLLLLLLLLLLLLL.LLLLLLLLL.LLLLLLLLLLLL
...
bd, any_changed = Board(data), True
while any_changed:
    bd, any_changed = bd.transform(transform_part1)
print("Part1: ", len(bd.find_elements(["#"])))

def next_full_seat(p,d,bd):
    while True:
        p = p +d
        if p not in bd:
            return None
...
def transform_part2(p, char, bd):
    s = []
    for d in DIRS_8:
        n = next_full_seat(p,d,bd)
        if n in bd:
            s.append(bd[n])
    c = Counter(s)
...
bd, any_changed = Board(data), True
while any_changed:
    bd, any_changed = bd.transform(transform_part2)
print("Part2: ",len(bd.find_elements(["#"])))
