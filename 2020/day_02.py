...
for line in data.split("\n"):
    parts = line.split(" ")
    rangebit = parts[0].split("-")
    lower = int(rangebit[0])
    upper = int(rangebit[1])
    ...
    if lower <= c[letter] <= upper:
        part_1 += 1
    if (passbit[lower - 1] == letter) != (passbit[upper - 1] == letter):
        part_2 += 1
