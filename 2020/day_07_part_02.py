...
data="""striped orange bags contain 1 vibrant green bag, 5 plaid yellow bags, 1 drab magenta bag.
...
for line in data.split("\n"):
    ...
    if content == "no other bags":
        ...
        for cont in content.split(", "):
            cont+=" "
            num, ty,_ = parse_line(cont, [Int()," ",Str()," bag",Str()])
            graph[bag].add((num,ty))
            
print(graph)
                                   
                   
def does_contain(bag):
...
def contains_how_many(bag):
    c = 1
    for n, ba in graph[bag]:
        
        c+=n*contains_how_many(ba)
    print(bag,c)
    return c
    

print(contains_how_many("shiny gold")-1)
